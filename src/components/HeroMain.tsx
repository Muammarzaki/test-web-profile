...
const HeroMain = () => {
    ...
    return (
        <div className="relative h-[500px] overflow-hidden">
            ...
            {/*    {slides.map((slide, index) => ( //TODO ini harus dibenerin*/}
            {/*        <button*/}
            {/*            key={slide.image}*/}
            ...
            <div className="absolute bottom-4 left-0 right-0 z-30 flex justify-center space-x-2">
                <button
                    className={`w-3 h-3 rounded-full ${currentSlide === 0 ? 'bg-white' : 'bg-white/50'}`}
                    onClick={() => prevSlide()}
                    disabled={currentSlide === 0}
                />
                <button
                    className={`w-3 h-3 rounded-full ${currentSlide === slides.length - 1 ? 'bg-white' : 'bg-white/50'}`}
                    onClick={() => nextSlide()}
